...
async function getCurrentVMSSNodeCount() {
	return getVMSSNodeCountAndState().then(() => {
		return currentVMSSNodeCount;
	});
}
...
function scaleSignalingWebServers(newCapacity) {
	...
	msRestNodeAuth.loginWithVmMSI(options).then((creds) => {
		...
		vmss.update(config.resourceGroup, config.virtualMachineScaleSet, updateOptions).then((result) => {
			console.log(`Success Scaling VMSS: ` + JSON.stringify(result));
...
function evaluateAutoScalePolicy(cirrusServers) {
	...
	getVMSSNodeCountAndState().then(() => {
		...
		// ai.logMetric("TotalStreams", totalInstances);
		...
		console.error(`Scaling evaluation failed - retrying next time... msg: ${err}`);
	});
}
