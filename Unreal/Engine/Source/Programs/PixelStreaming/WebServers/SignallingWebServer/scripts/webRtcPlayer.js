...
(function (root, factory) {
  ...
  function webRtcPlayer(parOptions, logger) {
    parOptions = typeof parOptions !== 'undefined' ? parOptions : {};

    var self = this;
    this.logger = logger || ApplicationInsightsLogger();
    ...
    if (!this.useMic) {
      ...
      self.logger.logInfo("Microphone access is not enabled. Pass ?useMic in the url to enable it.");
    }
    ...
    if (this.useMic && !isLocalhostConnection && !isHttpsConnection) {
      ...
      self.logger.logError("Microphone access in the browser will not work if you are not on HTTPS or localhost. Disabling mic access.");
      ...
      self.logger.logError("For testing you can enable HTTP microphone access Chrome by visiting chrome://flags/ and enabling 'unsafely-treat-insecure-origin-as-secure'");
    }
    ...
    onsignalingstatechange = function (state) {
      ...
      self.logger.logInfo('signaling state change:', state);
    };

    oniceconnectionstatechange = function (state) {
      console.info('ice connection state change:', state)
      self.logger.logInfo('ice connection state change:', state);
    };

    onicegatheringstatechange = function (state) {
      console.info('ice gathering state change:', state)
      self.logger.logInfo('ice gathering state change:', state);
    };

    handleOnTrack = function (e) {
      console.log('handleOnTrack', e.streams);
      self.logger.logInfo('handleOnTrack', e.streams);

      if (e.track) {
        console.log('Got track - ' + e.track.kind + ' id=' + e.track.id + ' readyState=' + e.track.readyState);
        self.logger.logInfo('Got track - ' + e.track.kind + ' id=' + e.track.id + ' readyState=' + e.track.readyState);
      }
      ...
      else (e.track.kind == "video" && self.video.srcObject !== e.streams[0])
      {
        ...
        self.logger.logInfo('Set video source from video track ontrack.');
        return;
      }
    ...
    handleOnAudioTrack = function (audioMediaStream) {
      ...
      // video element has some other media stream that is not associated with this audio track
        ...
        self.logger.logInfo('Created new audio element to play seperate audio stream.');
      }
    ...
    setupDataChannel = function (pc, label, options) {
      try {
        ...
        self.logger.logInfo(`Created datachannel (${label})`);
        ...
        datachannel.onopen = function (e) {
          ...
          self.logger.logInfo(`data channel (${label}) connect`);
        ...
        datachannel.onclose = function (e) {
          ...
          self.logger.logInfo(`data channel (${label}) closed`);
        }
        ...
        self.logger.logWarning('No data channel', e);
        return null;
      }
    }
    ...
    onicecandidate = function (e) {
      ...
      self.logger.logInfo('ICE candidate', e);
    ...
    handleCreateOffer = function (pc) {
      pc.createOffer(self.sdpConstraints).then(function (offer) {
        ...
        function () { console.warn("Couldn't create offer"); self.logger.logWarning("Couldn't create offer"); });
    }
    ...
    setupPeerConnection = function (pc) {
      if (pc.SetBitrate) {
        ...
        self.logger.logInfo("Hurray! there's RTCPeerConnection.SetBitrate function");
      }
    ...
    this.handleCandidateFromServer = function (iceCandidate) {
      ...
      self.logger.logInfo("ICE candidate: ", iceCandidate);
      let candidate = new RTCIceCandidate(iceCandidate);
      self.pcClient.addIceCandidate(candidate).then(_ => {
        console.log('ICE candidate successfully added');
        self.logger.logInfo('ICE candidate successfully added');
      });
    };
    ...
    this.createOffer = function () {
      if (self.pcClient) {
        ...
        self.logger.logInfo("Closing existing PeerConnection");
        self.pcClient.close();
        self.pcClient = null;
      }
    ...
    this.receiveAnswer = function (answer) {
      ...
      console.log(answer);
      self.logger.logInfo('Received answer:', answer);
    ...
    this.close = function () {
      if (self.pcClient) {
        ...
        self.logger.logInfo("Closing existing peerClient");
        self.pcClient.close();
        self.pcClient = null;
      }
